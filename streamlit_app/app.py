...
API_URL = os.getenv("https://crud-fastapi-39ag.onrender.com", "http://127.0.0.1:8000")
...
if btn_criar:
    ...
    res = requests.post(f"{"https://crud-fastapi-39ag.onrender.com"}/items/", json=payload)
...
if btn_atualizar and id_item > 0:
    payload = {"nome": nome, "preco": preco, "descricao": descricao}
    res = requests.put(f"{"https://crud-fastapi-39ag.onrender.com"}/items/{id_item}", json=payload)
    if res.status_code == 200:
...
if btn_deletar and id_item > 0:
    res = requests.delete(f"{"https://crud-fastapi-39ag.onrender.com"}/items/{id_item}")
    if res.status_code == 200:
...
try:
    resposta_get = requests.get(f"{"https://crud-fastapi-39ag.onrender.com"}/items/")
